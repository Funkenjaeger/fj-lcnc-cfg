...
""" %(os.path.join(paths.IMAGEDIR,'lcnc_swoop.png'))
    ...
    def main_tab_changed(self, btn):
        ...
        if index == TAB_MAIN:
            ...
            self.w.stackedWidget.setCurrentIndex(3) # EMD: the offsets pane has useless laser & tool setter functions, show the tool touchoff pane instead
        elif index == TAB_TOOL:
    ...
    def btn_touchoff_clicked(self):
        ...
        info = "Ensure tooltip is within {} inch of tool sensor and click OK".format(self.w.lineEdit_max_probe.text())
    ...
    def touchoff(self, selector):
        ...
        if ACTION.CALL_MDI_WAIT(command, 10) == -1:
            ACTION.CALL_MDI("G90")
            return
        if ACTION.CALL_MDI_WAIT("G1 Z0.05"):
            ACTION.CALL_MDI("G90")
            return
        ACTION.CALL_MDI("G4 P0.5")
        command = "G38.2 Z-0.1 F{}".format(probe_vel)
        if ACTION.CALL_MDI_WAIT(command, 10) == -1:
            ACTION.CALL_MDI("G90")
            return
        command = "G10 L20 P0 Z{}".format(z_offset)
        ACTION.CALL_MDI_WAIT(command)
        command = "G1 Z0.1 F{}".format(search_vel)
        ACTION.CALL_MDI_WAIT(command)
        ACTION.CALL_MDI("G90")
